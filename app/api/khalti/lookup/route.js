...
export async function POST(request) {
  try {
    ...
    const khaltiResponse = await fetch(`${getBaseUrl()}/api/v2/epayment/lookup/`, {
      method: 'POST',
      headers: {
    ...
    if (!khaltiResponse.ok) {
      return NextResponse.json(
        {
          error: result?.detail || 'Failed to verify Khalti payment.',
          khalti: result,
        },
    ...
    return NextResponse.json(
      { error: error?.message || 'Unexpected server error while verifying Khalti payment.' },
      { status: 500 }
    );
  }
}
