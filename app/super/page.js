...
const ROLE_OPTIONS = ['user', 'seller', 'delivery']; // keep super_admin protected
...
export default function AdminDashboard() {
  ...
  const loadUsers = async () => {
    setUsersError('');
    // NOTE: RLS must allow super_admin to read all profiles.
    const { data, error } = await supabase
      .from('profiles')
  ...
  const updateUserRole = async (userId, newRole) => {
    ...
    // prevent changing own role from UI
    if (adminProfile?.id === userId) {
      setUsersError("You can’t change your own role.");
      return;
    }
  ...
  if (checking) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <p className="text-gray-700">Loading super admin dashboard...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-orange-600">Super Admin Dashboard</h1>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-white p-6 rounded border border-gray-200">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold">{isEditing ? 'Edit Food Item' : 'Add Food Item'}</h2>
        ...
        <div className="bg-white p-6 rounded border border-gray-200 overflow-x-auto">
          <div className="flex items-center justify-between mb-4">
            ...
            <p className="text-sm text-gray-600">Showing {filteredItems.length} item(s)</p>
          </div>
        ...
        <div className="bg-white p-6 rounded border border-gray-200">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <h2 className="font-semibold">Users (Role Management)</h2>
          ...
          <p className="text-xs text-gray-500 mt-3">
            Note: Role changes work only if your Supabase RLS allows <b>super_admin</b> to update
            <code className="mx-1">profiles.role</code>.
          </p>
        </div>
      </div>
    </div>
